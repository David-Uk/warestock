...
> **Generated from:** [GitHub Issues](https://github.com/David-Uk/warestock/issues?q=is:open) — 38 open feature issues
...
The project already has 68 screens designed and built in Stitch, plus 3 design systems. The following screens still need to be generated to match the remaining open GitHub issues.
...
### Category A: Web Screens (13 screens)
...
**Prompt:** Generate a platform administration page for the WareStock AI web app. Use the WareStock Terminal design system. Display system settings including: user role management, warehouse configuration, notification preferences, API key management, and audit log access. Use `#FFFFFF` cards with `1px` solid `#E2E8F0` border and `8px` radius (`rounded-lg`). Section headers should use `label-caps` typography in `#44474d` on-text-variant. Include toggle switches for system features, configuration forms with `#FFFFFF` input fields (`2px` solid `#CBD5E1` border, `8px` radius), and save/cancel buttons. Primary action button: `#16283d` fill, `#FFFFFF` text, `8px` radius, `48px` minimum height.
...
### Category B: Mobile Screens (7 screens)
...
### Category C: Mobile Features (not screens, but related)

#### 21. Secure Token Storage (Issue #24)
**Prompt:** Generate a secure token storage configuration screen for the WareStock AI mobile app. Use the WareStock Terminal design system. Display a settings page showing current authentication status, token expiry, and security options. Include toggle switches for biometric login, auto-refresh tokens, and session timeout. Use `#FFFFFF` cards with `1px` solid `#C8C5BD` border. Status indicators should use `#3C8558` (green) for secure and `#E8A33D` (amber) for expiring. Include a "Re-authenticate" button with `#16283d` fill.

#### 22. Push Notification Handling (Issue #26)
**Prompt:** Generate a push notification settings screen for the WareStock AI mobile app. Use the WareStock Terminal design system. Display notification preferences grouped by category: Stock Alerts (`#C1443C`), Discrepancy Alerts (`#E8A33D`), Reorder Notifications (`#3C8558`), System Updates (`#44474d`). Each category should have a toggle switch with `48px` minimum touch target. Include a preview section showing sample notification messages styled with the WareStock Terminal design tokens. Use `#FFFFFF` surface with `1px` solid `#C8C5BD` border.
...
Build screens in this priority order:

### Phase 1 — Core Inventory (Highest Priority)
1. #27 Web Login Page
...
19. #26 Push Notification Handling
20. #39 CSV Export Functionality

### Phase 5 — Mobile & Infrastructure
21. #24 Secure Token Storage
...
| AI Prediction | `#F5F3FF` | `#8B5CF6` | `#6D28D9` |
...
| AI Assist | Gradient `#8B5CF6`→`#6366F1` | `#FFFFFF` | `4px` | `48px` |
